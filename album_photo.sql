...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : jeu. 23 avr. 2026 à 19:15
...
(2, 'Florian', 'florian.simon32320@gmail.com', '57f1a091860b520b0e3f04ab951b9b0b', 1),
...
-- Évènements
--
DROP EVENT IF EXISTS `purge_2m`$$
CREATE DEFINER=`root`@`localhost` EVENT `purge_2m` ON SCHEDULE EVERY 2 MINUTE STARTS '2026-03-31 09:11:04' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    -- Suppression de tout ce qui a plus de 2 minutes
    DELETE FROM commentaire 
    WHERE statut_suppression = 'ARCHIVE' 
    AND date_archivage < DATE_SUB(NOW(), INTERVAL 2 MINUTE);
